...
export function ExpenseList({ expenses, categories, compact = false, onDelete, onEdit, expandedReceiptId: requestedExpandedReceiptId, onReceiptOpen }: { expenses: Expense[]; categories: Category[]; compact?: boolean; onDelete?: (id: string) => void; onEdit?: (expense: Expense) => void; expandedReceiptId?: string | null; onReceiptOpen?: (expense: Expense) => void }) {
  ...
  function toggleReceipt(expense: Expense) { if (onReceiptOpen) onReceiptOpen(expense); else setLocalExpandedReceiptId((current) => current === expense.id ? null : expense.id) }
  ...
  return <div className={`expense-list ${compact ? 'compact' : ''}`}>{expenses.map((expense) => {
    ...
    return <div className={isReceipt ? 'expense-entry receipt-entry' : 'expense-entry'} key={expense.id}>
      <div className="expense-row" onClick={() => isReceipt && !compact && toggleReceipt(expense)}>{isReceipt ? <button className="expense-icon receipt-row-trigger" type="button" aria-label={`Pokaż szczegóły paragonu ${expense.merchant}`}><ReceiptText size={17} /></button> : <span className="expense-icon" style={{ color: category?.color, background: category ? `${category.color}18` : '#eef1f4' }}><Icon size={17} /></span>}<div className="expense-info"><strong>{expense.merchant}</strong><span>{dateFormatter.format(new Date(`${expense.date}T12:00:00`))} · {isReceipt ? `Paragon · ${expense.receipt?.itemCount} produktów · ${expense.receipt?.categoryBreakdown.length} kategorii` : category?.name ?? 'Bez kategorii'}</span></div>{isReceipt && !compact && <div className="receipt-category-chips">{expense.receipt?.categoryBreakdown.slice(0, 3).map((group) => <span key={group.categoryId ?? 'none'}>{categories.find((item) => item.id === group.categoryId)?.name ?? 'Bez kategorii'}</span>)}</div>}<strong className="expense-amount">−{moneyFormatter.format(expense.amount)}</strong>{isReceipt && !compact ? <button className="text-button receipt-toggle" type="button" onClick={(event) => { event.stopPropagation(); toggleReceipt(expense) }}>{isExpanded ? <>Zwiń <ChevronUp size={15} /></> : <>Szczegóły <ChevronDown size={15} /></>}</button> : compact ? onEdit ? <button className="details-button" type="button" onClick={() => onEdit(expense)} aria-label={`Edytuj wydatek ${expense.merchant}`}>Szczegóły</button> : null : <>{onEdit && <button className="edit-button" type="button" onClick={() => onEdit(expense)} aria-label={`Edytuj wydatek ${expense.merchant}`}><Pencil size={16} /></button>}{onDelete && <button className="delete-button" type="button" onClick={() => requestDelete(expense)} aria-label={`Usuń wydatek ${expense.merchant}`}><Trash2 size={16} /></button>}</>}</div>
