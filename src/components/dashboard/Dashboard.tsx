...
export function Dashboard({ expenses, categories, total, categoryTotals, onNavigate, onAddExpense, onOpenReceipt }: { expenses: Expense[]; categories: Category[]; total: number; categoryTotals: CategoryTotal[]; onNavigate: (view: View) => void; onAddExpense: () => void; onOpenReceipt: (id: string) => void }) {
  ...
  return <div className="page dashboard"><section className="page-heading"><div><span className="eyebrow">Twoje prywatne wydatki</span><h2>Przegląd finansów</h2></div><div className="month-selector"><ArrowLeft size={15} /><strong>Bieżący miesiąc</strong><ArrowRight size={15} /></div></section>
    ...
    {expenses.length === 0 ? <EmptyDashboard onAddExpense={onAddExpense} /> : <><section className="charts-grid"><SpendingChart expenses={expenses} total={total} /><CategoryChart categories={categoryTotals} total={total} /></section><section className="feature-banner"><span className="review-icon"><Sparkles size={19} /></span><div><strong>Rozpoznawanie paragonów jest dostępne</strong><span>Dodaj prywatne zdjęcie, uruchom lokalny OCR i zatwierdź wynik jako wydatek.</span></div><button className="button secondary" onClick={() => onNavigate('receipts')}>Dodaj paragon</button></section><RecentExpenses expenses={expenses} categories={categories} onNavigate={onNavigate} onOpenReceipt={onOpenReceipt} /></>}
  </div>
}
...
function RecentExpenses({ expenses, categories, onNavigate, onOpenReceipt }: { expenses: Expense[]; categories: Category[]; onNavigate: (view: View) => void; onOpenReceipt: (id: string) => void }) {
  return <section className="recent-section"><div className="section-header"><div><h3>Ostatnie wydatki</h3><span>Najnowsze zapisane pozycje</span></div><button className="text-button" onClick={() => onNavigate('expenses')}>Wszystkie wydatki <ArrowRight size={15} /></button></div><ExpenseList expenses={expenses.slice(0, 4)} categories={categories} compact onEdit={(expense) => expense.receipt ? onOpenReceipt(expense.id) : onNavigate('expenses')} /></section>
}
