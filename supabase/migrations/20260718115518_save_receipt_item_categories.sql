...
begin
  ...
  if char_length(trim(coalesce(p_merchant, ''))) not between 1 and 160 or p_purchased_at is null or p_total_amount is null or p_total_amount <= 0 then
  ...
  if p_category_id is not null and not exists (select 1 from public.categories where id = p_category_id and user_id = v_user_id) then
    raise exception 'Category not found';
  end if;

  update public.receipts set merchant = trim(p_merchant), purchased_at = p_purchased_at, total_amount = p_total_amount, category_id = p_category_id, status = 'needs_review', updated_at = now()
  where id = p_receipt_id and user_id = v_user_id and status in ('needs_review', 'failed');
  ...
  loop
    ...
    insert into public.receipt_items (receipt_id, user_id, category_id, line_number, name, quantity, unit_price, total_price)
    values (p_receipt_id, v_user_id, v_item_category_id, v_line_number, trim(v_item ->> 'name'), nullif(v_item ->> 'quantity', '')::numeric, nullif(v_item ->> 'unitPrice', '')::numeric, (v_item ->> 'totalPrice')::numeric);
